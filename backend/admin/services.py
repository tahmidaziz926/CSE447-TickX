...
_EXCLUDED_FIELDS = {
    ...
    "rsa_private_key": 0, "ecc_private_key": 0,
}


def list_users(role: str = None) -> list:
    query = {"role": role} if role else {}
    users = list(users_collection().find(query, _EXCLUDED_FIELDS))
    for u in users:
        u["_id"] = str(u["_id"])
    return users
...
def list_pending_sellers() -> list:
    sellers = list(users_collection().find(
        {"role": "SELLER", "seller_status": "PENDING"}, _EXCLUDED_FIELDS,
    ))
    for s in sellers:
        s["_id"] = str(s["_id"])
    return sellers
...
def approve_seller(seller_id, admin_id) -> None:
    oid = ObjectId(seller_id) if isinstance(seller_id, str) else seller_id
    result = users_collection().update_one(
        {"_id": oid, "role": "SELLER"}, {"$set": {"seller_status": "APPROVED"}},
    )
    if result.matched_count == 0:
        raise AdminError("seller not found")
    log_security_event("SELLER_APPROVED", f"Seller {seller_id} approved", {"admin_id": str(admin_id)})


def reject_seller(seller_id, admin_id) -> None:
    oid = ObjectId(seller_id) if isinstance(seller_id, str) else seller_id
    result = users_collection().update_one(
        {"_id": oid, "role": "SELLER"}, {"$set": {"seller_status": "REJECTED"}},
    )
    if result.matched_count == 0:
        raise AdminError("seller not found")
    log_security_event("SELLER_REJECTED", f"Seller {seller_id} rejected", {"admin_id": str(admin_id)})


def suspend_account(user_id, admin_id) -> None:
    oid = ObjectId(user_id) if isinstance(user_id, str) else user_id
    result = users_collection().update_one({"_id": oid}, {"$set": {"account_status": "SUSPENDED"}})
    if result.matched_count == 0:
        raise AdminError("user not found")
    log_security_event("ACCOUNT_SUSPENDED", f"User {user_id} suspended", {"admin_id": str(admin_id)})


def reactivate_account(user_id, admin_id) -> None:
    oid = ObjectId(user_id) if isinstance(user_id, str) else user_id
    result = users_collection().update_one({"_id": oid}, {"$set": {"account_status": "ACTIVE"}})
    if result.matched_count == 0:
        raise AdminError("user not found")
    log_security_event("ACCOUNT_REACTIVATED", f"User {user_id} reactivated", {"admin_id": str(admin_id)})
...
def admin_deactivate_event(event_id, admin_id) -> None:
    ...
    log_security_event("EVENT_DEACTIVATED_BY_ADMIN", f"Event {event_id} deactivated", {"admin_id": str(admin_id)})
