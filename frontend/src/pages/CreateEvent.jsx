...
/**
 * CreateEvent.jsx — seller event creation (Section 3.2).
 * Backend rejects this for PENDING/REJECTED sellers, so the error
 * message from a non-approved seller surfaces here.
 */
export default function CreateEvent() {
  const { isAuthenticated, role, logout, token } = useAuth();
  const [form, setForm] = useState({
    name: "", category: "Music", date_time: "", venue: "",
    description: "", ticket_price: "", total_seats: "",
  });
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      setSuccess(true);
      setTimeout(() => { window.location.href = "/dashboard/events"; }, 1200);
    } catch {
      ...
      <main className="auth-page">
        <div className="ticket-card auth-card auth-card--wide">
          ...
          <p className="auth-card__sub">
            Your event is ECC-signed on creation so any later tampering can be detected.
          </p>

          {success ? (
            <p className="create-event__success">Event created — redirecting…</p>
          ) : (
